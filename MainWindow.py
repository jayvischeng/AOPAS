...
#数据排序
		...
		self.tableView.customContextMenuRequested.connect(functools.partial(self.dbRightClicked,self.tableView))
		...
		self.A = ["近地面测量系统", "湍流探测激光雷达", "Airda16000低平流层风廓线雷达", "全天空成像仪", "大孔径闪烁仪",
				  "POM-02太阳辐射计", "DTF太阳辐射计", "PGS太阳辐射计", "温度脉动仪",
				  "自动气象站,CO2传感器", "常规气象参数测量系统(35mTower,华云技术AWS)", "图像采集系统", "光纤湍流测量系统",
				  "大气能见度仪", "光学粒子计数器(OPC)", "微脉冲激光雷达(MPL)", "光纤光谱仪",
				  "APS3321", "炭黑仪(AE31),烛度计(3563)", "手持太阳辐射计", "车载风雷达(WP3000)",
				  "大气相干长度仪", "天空亮度仪"]

		self.B = ["JDMCL", "RTSL", "Airda16000", "TSI880", "LALS",
				  "POM02", "DTF", "PGS", "QHTP",
		...
		self.tablenamedict = dict(zip(self.A, self.B))
		...
		#---------------------------------------------------------
		saveout = sys.stdout
		sys.stdout = OutLog(self.console)
...
#数据查询
	def Query(self,action):
		...
		try:
			a = unicode(self.dcombox_name.currentText()).encode("utf-8")
 ...
 #数据打印
	...
	def on_dcombox_name_currentIndexChanged(self, p0):
		if self.dcombox_name.currentIndex() > 0:
			self.tablename = self.tablenamedict[unicode(p0).encode("utf-8")]
			sqlstr = "select * from " + self.tablename+" where Second=0"
			...
			try:
				if self.ms.GetConnect() == True:
					for tab in range(len(self.ms.ExecQuery(sqlstr))):
						tempresult.append(unicode(self.ms.ExecQuery(sqlstr)[tab][1]).encode("utf-8").decode("utf-8"))
				#self.dcombox_index.addItems(self.ms.title)
