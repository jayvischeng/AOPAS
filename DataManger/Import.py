...
#sys.path.append("F:\MyGit1\MyGit1AOPAS")
...
class Import(QDialog, Ui_Import):
	...
	def __init__(self, parent=None):
		...
		self.idcount = self.conninfo.maxID
		try:
	...
	def on_pbImport_clicked(self):
		...
		if len(self.files) != 0 and len(self.folder) == 0:
			...
			for tab1 in range(len(self.filepath)):
				...
				#扫描每个文件的第一行，判断是否有时间列和日期列
				line_list = re.split(r"\t *|,| *", str_file[0].strip())
				#print line_list
				dateIndex = -1
				timeIndex = -1
				for tab2 in range(len(line_list)):
					if line_list[tab2].count('-') == 2:
						dateIndex = tab2
					elif line_list[tab2].count(':') == 2:
						timeIndex = tab2
				...
				for tab3 in range(len(str_file)):
					...
					sqlstr2 = "insert into "+self.tablename
	...
	def on_pbAdd_clicked(self):
		self.listWidget.addItem(str(self.comboBox.currentText()) + '   ' + self.index1.text() + '   ' + self.index2.text())
